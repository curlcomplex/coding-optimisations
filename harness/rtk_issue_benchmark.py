...
def run(c,w,e,t=180):return subprocess.run(c,cwd=w,env=e,text=True,capture_output=True,timeout=t)
def usage(home,tid):
 f=list(home.glob('sessions/**/*'+tid+'*.jsonl'))
 if len(f)!=1:return None
 total=None;req=0
 for line in f[0].read_text(errors='replace').splitlines():
  ...
  if r.get('type')=='event_msg' and p.get('type')=='token_count' and p.get('info'):
   ...
   if t and t!=total:total=t;req+=bool(last)
 return {'total':total,'requests':req}
def fixture(root,kind):
 root.mkdir();src,test,prompt=TASKS[kind]
 if kind=='cpp':(root/'voice.cpp').write_text(src);(root/'test.cpp').write_text(test);tc=['sh','-lc','clang++ -std=c++17 voice.cpp test.cpp -o test && ./test']
 else:(root/'logic.mjs').write_text(src);(root/'test.mjs').write_text(test);tc=['node','test.mjs']
...
def arm(codex,home,root,kind,rep,hook,hookbin):
 cwd=root/(f'{kind}-{rep}-'+('on' if hook else 'off'));prompt,tc=fixture(cwd,kind);env=os.environ.copy();[env.pop(k,None) for k in ('OPENAI_API_KEY','CODEX_API_KEY','OPENAI_BASE_URL')];marker='BENCH_'+uuid.uuid4().hex
 ...
 test=run(tc,cwd,env,60);diff=run(['git','diff','--check'],cwd,env,30);u=usage(home,tid) if tid else None
